...
const NATIONAL_WIRE_OVERRIDE_RE =
  /\b(?:
    ...
    new\s+orleans\s*[—-]\s*|
    (?:ap|reuters|afp)\s*[—-]\s*|
    the\s+associated\s+press\s*[—-]|
    nbc\s+news\s*[—-]|
    cnn\s*[—-]|
    abc\s+news\s*[—-]|
    cbs\s+news\s*[—-]|
    fox\s+news\s*[—-]|
    dubai\s*[—-]\s*united\s+arab|
    from\s+(?:new\s+york|washington|london|dubai|tel\s+aviv|jerusalem|paris|berlin|beijing|moscow|tokyo)|
    \(anf(?:\/gray\s+news)?\)\s*[—-]?\s*|
    \(gray\s+news\)\s*[—-]?\s*|
    \(nexstar\s+media\s+wire\)\s*[—-]?\s*|
    \(cnn\s+newsource\)\s*[—-]?\s*
  )/ix;
